...
curl "localhost:8123" -d "DROP TABLE IF EXISTS test_prom_metric_series ON CLUSTER abc SYNC"
curl "localhost:8123" -d "CREATE TABLE test_prom_metric_series ON CLUSTER abc
(
    __series_id Int64,
    __mgmt_id Int64,
    labels String,
...
curl "localhost:8123" -d "DROP TABLE IF EXISTS dist_test_prom_metric_series ON CLUSTER abc SYNC"
curl "localhost:8123" -d "CREATE TABLE dist_test_prom_metric_series ON CLUSTER abc AS test_prom_metric_series ENGINE = Distributed(abc, default, test_prom_metric_series);"
...
[ $schema = "__series_idInt64,timestampDateTimeDoubleDelta,LZ4,valueFloat32ZSTD(15)," ] || exit 1


schema=`curl "localhost:8123" -d 'DESC test_prom_metric_series' 2>/dev/null | sort | tr -d '\t' | tr -d ' '| tr '\n' ','`
echo "Got test_prom_metric_series schema => $schema"
...
count=`curl "localhost:8123" -d 'select count() from dist_test_prom_metric_series'`
echo "Got test_prom_metric_series count => $count"
